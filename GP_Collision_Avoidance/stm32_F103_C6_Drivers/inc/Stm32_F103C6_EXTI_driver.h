...
typedef struct{
	...
	uint8_t 				Trigger_case;	 //Specifies Raising , Falling or both trigger
										 	 //This parameter must be set based on @ref EXTI_Trigger_define

	uint8_t 				IRQ_EN ;		 //Enable or Disable the EXTI(That will enable the IQR MASK in EXTI and Also on The NVIC Interrupt Controller)
